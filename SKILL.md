...
and any other tool via the `SessionSource` plugin protocol.
...
```bash
python -m ai_code_cognitive_stress --month YYYY-MM   --output <path>
python -m ai_code_cognitive_stress --year  YYYY      --output <path>
python -m ai_code_cognitive_stress --day   YYYY-MM-DD --output <path>
```
...
```bash
python -m ai_code_cognitive_stress --year 2026 --output ~/stress-2026.html
```
...
```bash
python -m ai_code_cognitive_stress --export-research --year 2026 --i-consent \
...
The `--export-research` path only **writes a file**; the user uploads it
manually if they choose. Session transcripts often contain proprietary code
and credentials; treat the generated HTML as similarly sensitive.
