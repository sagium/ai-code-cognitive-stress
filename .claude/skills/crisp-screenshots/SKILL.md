...
- **The app's theme is the source of truth.** Dark report surfaces use the
  palette in `stress_levels/render.py` `_STYLES` (`--bg: #161814`, ink
  `rgba(245,243,237,.92)`, accent `#efe9da`, panel gradient
  `178deg, #1d201c → #131512`, glass edges `rgba(255,255,255,.13)`); zone
  colours come from `stress_levels/scales.py` (`#6c9a8b` / `#c5b48a` /
...
- Copy on artifacts follows the repo's wording rules: "private" (never
  "local-only"), no coding-tool vendor names, no emoji.
