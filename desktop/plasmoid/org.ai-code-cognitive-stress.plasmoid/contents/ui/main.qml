...
PlasmoidItem {
    ...
    Plasma5Support.DataSource {
        ...
        onNewData: function (source, data) {
            ...
            if (source.indexOf("--set-view") !== -1) {
                // The tab view was persisted; the client already switched the
                // view, so no re-render needed (and no card-validation error).
                return
            }
            if (source.indexOf("--rate") !== -1 || source.indexOf("--set-compact") !== -1) {
                // A grade (`--rate`) or compact toggle (`--set-compact`) was just
                // recorded; its output is not a card, so skip the card-validation
                // (which would otherwise show "did not print a widget card") and
                // refresh so the grader collapses / the card re-renders at the new
                // size.
                // Force-disconnect the main command first: if the 10s timer had it
                // connected and still running, a plain connectSource() would be a
                // no-op and the re-render would be delayed up to 10 seconds.
                executable.disconnectSource(Plasmoid.configuration.command)
                executable.run(Plasmoid.configuration.command)
                return
            }
            ...
            WebEngineView {
                ...
                onLoadingChanged: function (loadingInfo) {
                    if (loadingInfo.status === WebEngineView.LoadSucceededStatus) {
                        ...
                        // Inject click handlers via runJavaScript so they are
                        // guaranteed to run. The card's embedded <script> covers
                        // the same cases, but QtWebEngine's loadHtml() does not
                        // reliably execute inline scripts — confirmed by grade
                        // sidecars never being written despite chip clicks.
                        // runJavaScript() is called explicitly by QML so it always
                        // runs. The cogwired guard (dataset.cogwired) makes this
                        // single-shot: whichever wires first wins (inline script or
                        // this injected handler), so clicks never double-fire.
                        runJavaScript("(function(){" +
                            "var root=document.querySelector('.cogstress');" +
                            "if(!root)return;" +
                            // Idempotency guard: whichever wires first (inline script
                            // or this injection) wins; the second is a no-op.
                            "if(root.dataset.cogwired)return;" +
                            "root.dataset.cogwired='1';" +
                            "root.addEventListener('click',function(e){" +
                                // Grade chip → title bridge → onTitleChanged → --rate
                                "var chip=e.target.closest('.grade-chip');" +
                                "if(chip){" +
                                    "var day=chip.getAttribute('data-day');" +
                                    "var grade=chip.getAttribute('data-grade');" +
                                    "if(day&&grade!==null){" +
                                        "root.removeAttribute('data-ungraded');" +
                                        // Instantly mark the chosen chip .sel so the user sees
                                        // feedback before the CLI re-render arrives.
                                        "chip.closest('.grader-chips').querySelectorAll('.grade-chip').forEach(function(c){c.classList.toggle('sel',c===chip);});" +
                                        "document.title='cogstress:rate:'+day+':'+grade+':'+Date.now();" +
                                        "setTimeout(function(){var h=Math.ceil(root.getBoundingClientRect().height);if(h>0)document.title='cogstress:h:'+h;},50);" +
                                    "}" +
                                    "return;" +
                                "}" +
                                // Resize toggle → DOM flip + title bridge → --set-compact
                                "var rt=e.target.closest('.resize-toggle');" +
                                "if(rt){" +
                                    "var wc=root.getAttribute('data-compact')!=='true';" +
                                    "root.setAttribute('data-compact',wc?'true':'false');" +
                                    "document.title='cogstress:compact:'+(wc?'1':'0')+':'+Date.now();" +
                                    "setTimeout(function(){var h=Math.ceil(root.getBoundingClientRect().height);if(h>0)document.title='cogstress:h:'+h;},60);" +
                                    "return;" +
                                "}" +
                                // Tab switch → DOM toggle + view persist + height report
                                "var tab=e.target.closest('.tab');" +
                                "if(!tab)return;" +
                                "var key=tab.getAttribute('data-view');" +
                                "root.querySelectorAll('.tab').forEach(function(t){t.classList.toggle('active',t===tab);});" +
                                "root.querySelectorAll('.view').forEach(function(v){v.classList.toggle('hidden',v.getAttribute('data-view')!==key);});" +
                                "document.title='cogstress:view:'+key+':'+Date.now();" +
                                "setTimeout(function(){var h=Math.ceil(root.getBoundingClientRect().height);if(h>0)document.title='cogstress:h:'+h;},30);" +
                            "});" +
                        "})()");
                    }
                }

                // Height bridge: 'cogstress:h:<px>' on load and on tab/resize
                // changes so the widget grows/shrinks without a CLI round-trip.
                //
                // Grade bridge: 'cogstress:rate:<day>:<grade>:<nonce>' invokes
                // `aicogstress --rate <day>:<grade>`. onNewData immediately
                // re-runs --emit-html-card so the chip renders as selected.
                onTitleChanged: {
                    // Grade first — pattern: cogstress:rate:<day>:<grade>:<nonce>
                    var rm = /^cogstress:rate:(\d{4}-\d{2}-\d{2}):([0-2])(?::\d+)?$/.exec(cardView.title);
                    if (rm) {
                        var rateDay   = rm[1];
                        var rateGrade = rm[2];
                        // Derive the aicogstress binary from the configured command.
                        // Plasmoid.configuration.command is the full --emit-html-card
                        // invocation; split off the first token as the binary name.
                        var binary = Plasmoid.configuration.command.split(" ")[0];
                        executable.run(binary + " --rate " + rateDay + ":" + rateGrade);
                        return;
                    }
                    // Compact toggle — pattern: cogstress:compact:<0|1>[:<nonce>].
                    // Persist via `aicogstress --set-compact <true|false>`; the
                    // onNewData handler re-runs the card command so the widget
                    // re-renders (and the height bridge resizes it) immediately.
                    var cm = /^cogstress:compact:([01])(?::\d+)?$/.exec(cardView.title);
                    if (cm) {
                        var cbin = Plasmoid.configuration.command.split(" ")[0];
                        executable.run(cbin + " --set-compact " + (cm[1] === "1" ? "true" : "false"));
                        return;
                    }
                    // View tab persist — pattern: cogstress:view:<key>[:<nonce>].
                    // Persist via `aicogstress --set-view <key>`; onNewData
                    // short-circuits for --set-view (no re-render needed — the
                    // client already switched the tab; next periodic refresh picks
                    // up the persisted view).
                    var vm = /^cogstress:view:(today|week|month|year)(?::\d+)?$/.exec(cardView.title);
                    if (vm) {
                        var vbin = Plasmoid.configuration.command.split(" ")[0];
                        executable.run(vbin + " --set-view " + vm[1]);
                        return;
                    }
                    // Height bridge
                    var hm = /^cogstress:h:(\d+)$/.exec(cardView.title);
                    if (hm) {
