...
(see the corresponding caveat in \S\ref{sec:threats}). The bands share the
personal optimum's calibration bar of 14 active days (\S\ref{sec:optimum}):
below that count the distribution is too thin to rank a day against, so the
status scale falls back to fixed absolute cutoffs at 40 and 70 on the
$0$--$100$ composite.
