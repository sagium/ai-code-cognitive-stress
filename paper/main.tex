...
\begin{axis}[width=\columnwidth, height=5.2cm,
  axis lines=left, axis line style={black!60}, tick style={black!60},
  label style={font=\scriptsize}, tick label style={font=\scriptsize},
  xlabel={active day (calendar order)}, ylabel={daily composite},
  xmin=0.3, xmax=21.4, ymin=0, ymax=100,
  xtick={1,5,10,15,19}, ytick={0,25,50,75,100},
  ybar stacked, bar width=2.4mm,
  legend style={font=\scriptsize, draw=none, fill=none,
                at={(0.02,0.98)}, anchor=north west, area legend},
  xlabel near ticks, ylabel near ticks]
\addplot[fill=black!45, draw=black!70] coordinates
  {(1,54.5) (2,39.8) (3,49.9) (4,38.3) (5,59.0) (6,38.6) (7,61.7) (8,39.3)
   (9,42.4) (10,48.9) (11,73.2) (12,40.1) (13,48.2) (14,67.7) (15,57.0)
   (16,52.3) (17,39.1) (18,60.9) (19,55.6)};
\addlegendentry{in-window base}
\addplot[fill=black!15, draw=black!60] coordinates
  {(1,0) (2,0) (3,0) (4,7.7) (5,0) (6,0) (7,0) (8,0) (9,0) (10,0) (11,0)
   (12,0) (13,17.7) (14,0) (15,0) (16,0) (17,0) (18,0) (19,0)};
\addlegendentry{off-hours toll}
\draw[dashed, black!45] (axis cs:0.3,48.4) -- (axis cs:19.6,48.4);
\draw[dashed, black!45] (axis cs:0.3,57.9) -- (axis cs:19.6,57.9);
\draw[dashed, black!45] (axis cs:0.3,69.0) -- (axis cs:19.6,69.0);
\node[anchor=west, font=\tiny, inner sep=1pt] at (axis cs:19.6,48.4) {$p_{50}$};
\node[anchor=west, font=\tiny, inner sep=1pt] at (axis cs:19.6,57.9) {$p_{75}$};
\node[anchor=west, font=\tiny, inner sep=1pt] at (axis cs:19.6,69.0) {$p_{90}$};
\end{axis}
\end{tikzpicture}
\caption{One month of \emph{synthetic sample data} (19 active days, plotted in
calendar order). The data is the deterministic demonstration dataset shipped
with the reference implementation, generated without touching any real session
logs; it illustrates the shape of the tool's output while exposing no
operator's activity. Each bar is a day's composite; the light segment on top is
the additive off-hours toll (\S\ref{sec:composite}) on days that incurred one.
Dashed lines mark the $p_{50}$/$p_{75}$/$p_{90}$ of the surrounding synthetic
baseline, the same reference bands the report colors each day against. Being
synthetic, the month is an illustration of the instrument's output, not
evidence of validity (\S\ref{sec:threats}).}
\label{fig:month}
\end{figure}

To make the axes concrete, Figure~\ref{fig:month} shows one month of the
synthetic demonstration dataset that ships with the reference implementation:
19 active days, generated without reading any real session logs, so it exposes
no operator's activity. This is emphatically an \emph{illustration of the
instrument's output}, not evidence of validity; the data is synthetic and there
is no felt-load measure attached to it. The month shows the axes moving
independently, which is the behaviour the decomposition intends. The
highest-scoring day (composite $73.2$) is driven by an interruption rate near
the normalization ceiling on top of a saturated capacity-dose, while the
second-highest day ($65.9$) reaches its score by a different route: an in-window
base near the median lifted well past the $p_{75}$ band by the month's largest
off-hours engagement ($53$ minutes past the window, the $17.7$-point toll drawn
as the upper segment). A lighter day (base $38.3$, below the median) is likewise
carried up to near the median by a smaller off-hours toll, showing the additive
off-hours term moving a score independently of the three in-window axes. Whether any of this tracks \emph{felt} load is exactly
the open question of \S\ref{sec:validation}, and no synthetic dataset can
answer it. That unresolved criterion motivates the threats considered next.
...
capacity ($\kappa\approx 4$~\cite{cowan2001}), the CODL dose horizon ($D$), and
the interruption ceiling ($10$/hour) are literature-anchored priors, not fitted
to the population of agentic-coding software engineers, who may differ. No
empirical usage is reported at all: the illustrative example (\S\ref{sec:example})
is the reference implementation's own synthetic demonstration dataset, chosen so
the method can be shown without exposing any operator's session data, and it
carries no felt-load criterion against which the thresholds could be checked. The
...
the thresholds stay borrowed and no field data has been collected
(\S\ref{sec:validation}).
