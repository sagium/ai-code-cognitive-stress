...
design choice weights concurrency by \emph{engagement}: a session counts at full
...
Large-language-model coding assistants are routinely run several at a time, or in many concurrent
...
completion in a sitting---information $C(t)$ cannot carry. The gap is, however,
a \emph{proxy} for a
...
Equal weights are the explicit \emph{null hypothesis}: we have
...
most loops, rather than rediscovering the load band with the least concurrency. It requires at least 14
...
its own limitations, which we state plainly. First, the gap is a \emph{proxy}: it is
...
is added to the tool---is supported end-to-end by the released tooling (the
...
limitations, converting this from a principled instrument into a validated one.
