...
cognitive cost. We present a method and an open, \emph{private} tool that
...
\begin{enumerate}
  ...
  \item An open, dependency-free, private reference implementation that
...
The implementation also provides two optional, private pathways toward the
...
tool itself transmits nothing, preserving the privacy invariant. The second
...
multi-developer sample, each participant running the private tool on their
...
running the private tool on their own machine, with only the derived per-day
...
private processing as a hard invariant, not a feature, and any future
