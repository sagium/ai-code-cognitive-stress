...
from ai_code_cognitive_stress.__main__ import (
    _build_parser,
    _clear_dir,
    _open_in_browser,
    _parse_range,
...
def test_baseline_days_defaults_to_90():
    parser = _build_parser()
    args = parser.parse_args([])
    assert args.baseline_days == 90
...
def test_reset_archive_flag_parses_default_false():
    args = _build_parser().parse_args([])
    assert args.reset_archive is False
    assert _build_parser().parse_args(["--reset-archive"]).reset_archive is True


def test_main_reset_archive_clears_archive_before_running(tmp_path, monkeypatch):
    _isolate_pipeline(monkeypatch, tmp_path)
    import ai_code_cognitive_stress.__main__ as cli
    cleared: list = []
    monkeypatch.setattr(
        cli, "_clear_dir",
        lambda target, label, noun: cleared.append(noun) or True,
    )
    rc = main(["--reset-archive", "--month", "2026-05",
               "-o", str(tmp_path / "r.html")])
    assert rc == 0
    assert cleared == ["archive"]


def test_clear_dir_removes_existing_dir(tmp_path):
    cache = tmp_path / "cache"
    (cache / "v3" / "2026").mkdir(parents=True)
    (cache / "v3" / "2026" / "2026-05-29.json").write_text("{}", encoding="utf-8")
    assert _clear_dir(cache, "rebuild-cache", "cache") is True
    assert not cache.exists()


def test_clear_dir_missing_dir_is_noop(tmp_path):
    assert _clear_dir(tmp_path / "nope", "rebuild-cache", "cache") is False


def test_clear_dir_reports_oserror_without_aborting(tmp_path, monkeypatch, capsys):
    import shutil
    cache = tmp_path / "cache"
    cache.mkdir()
    ...
    assert _clear_dir(cache, "rebuild-cache", "cache") is False
...
def _isolate_pipeline(monkeypatch, tmp_path):
    ...
    def fake(since, until, **kw):
        ...
        # main() passes the real-home archive dir (DEFAULT_DATA_DIR is resolved
        # at import, before Path.home is patched). Route it to tmp so the CLI
        # tests stay hermetic and write nothing under the real home.
...
def test_main_defaults_to_all_available_sources(tmp_path, monkeypatch, capsys):
    _isolate_pipeline(monkeypatch, tmp_path)
    (tmp_path / "home" / ".claude" / "projects").mkdir(parents=True)
    (tmp_path / "home" / ".codex" / "sessions").mkdir(parents=True)

    rc = main(["--month", "2026-05", "-o", str(tmp_path / "r.html")])

    assert rc == 0
    assert "sources: claude-code, codex" in capsys.readouterr().err
...
def test_main_emit_json_prints_dayview(monkeypatch, capsys):
    import ai_code_cognitive_stress.output.dayview as dayview_mod
...
def test_main_set_compact_persists_and_exits(tmp_path, monkeypatch, capsys, value, expected):
    ...
    rc = main(["--set-compact", value])
    assert rc == 0
    assert json.loads(runtime.read_text(encoding="utf-8"))["compact_widget"] is expected
    assert f"compact_widget = {str(expected).lower()}" in capsys.readouterr().err


def test_main_set_compact_toggle_flips_current(tmp_path, monkeypatch):
    import ai_code_cognitive_stress.core.config as cfgmod
    runtime = tmp_path / "config.json"
    runtime.write_text(json.dumps({"compact_widget": False}), encoding="utf-8")
    monkeypatch.setattr(cfgmod, "_RUNTIME_CONFIG_PATH", runtime)
    cfgmod._CONFIG_CACHE.clear()

    assert main(["--set-compact", "toggle"]) == 0
    assert json.loads(runtime.read_text(encoding="utf-8"))["compact_widget"] is True


@pytest.mark.parametrize("view", ["today", "week", "month", "year"])
def test_main_set_view_persists_and_exits(tmp_path, monkeypatch, capsys, view):
    import ai_code_cognitive_stress.core.config as cfgmod
    ...
    rc = main(["--set-view", view])
    assert rc == 0
    cfgmod._CONFIG_CACHE.clear()
    assert load_config(runtime).widget_view == view
    assert f"widget_view = {view}" in capsys.readouterr().err


def test_main_set_view_rejects_invalid():
    parser = _build_parser()
    with pytest.raises(SystemExit):
        parser.parse_args(["--set-view", "quarterly"])
...
def test_main_rebuild_cache_clears_before_running(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        cli, "_clear_dir",
        lambda target, label, noun: cleared.append(target) or True,
    )
