...
def test_single_long_tool_run_is_not_a_resume():
    """One prompt → one long tool call: a tool_use then its tool_result 45 min
    later, nothing between. The span is autonomous tool execution, not a parked
    loop, so it must NOT be recorded as a resume gap."""
    events = [
        UserMessageEvent(ts=_utc(2026, 5, 15, 9, 0), stream_id="s1", project="p"),
        AssistantMessageEvent(ts=_utc(2026, 5, 15, 9, 0), stream_id="s1", project="p"),
        ToolUseEvent(ts=_utc(2026, 5, 15, 9, 0), stream_id="s1",
                     project="p", tool_name="Bash"),
        ToolResultEvent(ts=_utc(2026, 5, 15, 9, 45), stream_id="s1",
                        project="p", is_error=False),  # tool ran 45 min
    ]
    agg = _aggregate_events(date(2026, 5, 15), events)
    assert agg.streams[0].resume_gaps == ()


def test_human_park_after_tool_run_is_still_a_resume():
    """A long tool run (excluded) followed by the operator stepping away and
    returning: the tool span is dropped, but the operator's own return — a gap
    whose resuming event is their message — is still counted."""
    events = [
        ToolUseEvent(ts=_utc(2026, 5, 15, 9, 0), stream_id="s1",
                     project="p", tool_name="Bash"),
        ToolResultEvent(ts=_utc(2026, 5, 15, 9, 45), stream_id="s1",
                        project="p", is_error=False),   # 45-min tool span → dropped
        UserMessageEvent(ts=_utc(2026, 5, 15, 10, 25), stream_id="s1",
                         project="p"),                   # 40-min human park → kept
    ]
    agg = _aggregate_events(date(2026, 5, 15), events)
    assert agg.streams[0].resume_gaps == ((_utc(2026, 5, 15, 10, 25), 2400),)


def test_gap_resumed_by_human_message_is_a_resume():
    """Control: a long idle span whose resuming event is a user message (the
    operator parked and came back) is recorded as before."""
    events = [
        UserMessageEvent(ts=_utc(2026, 5, 15, 9, 0), stream_id="s1", project="p"),
        UserMessageEvent(ts=_utc(2026, 5, 15, 9, 40), stream_id="s1", project="p"),
    ]
    agg = _aggregate_events(date(2026, 5, 15), events)
    assert agg.streams[0].resume_gaps == ((_utc(2026, 5, 15, 9, 40), 2400),)


def test_parallel_tool_results_each_excluded():
    """Two tools fired in one assistant turn (same ts), results returning at
    different times: every gap whose resuming event is a tool_result is an
    autonomous span and is excluded, even when one result resumes after
    another result (not a tool_use)."""
    events = [
        ToolUseEvent(ts=_utc(2026, 5, 15, 9, 0), stream_id="s1",
                     project="p", tool_name="A"),
        ToolUseEvent(ts=_utc(2026, 5, 15, 9, 0), stream_id="s1",
                     project="p", tool_name="B"),
        ToolResultEvent(ts=_utc(2026, 5, 15, 9, 40), stream_id="s1",
                        project="p", is_error=False),   # A returns
        ToolResultEvent(ts=_utc(2026, 5, 15, 10, 25), stream_id="s1",
                        project="p", is_error=False),   # B returns 45 min later
    ]
    agg = _aggregate_events(date(2026, 5, 15), events)
    assert agg.streams[0].resume_gaps == ()
...
def test_tool_error_timestamps_captured_sorted_and_error_only():
    """Only errored results contribute a timestamp; non-error results don't.
    Timestamps are stored sorted regardless of event order."""
    events = [
        ToolResultEvent(ts=_utc(2026, 5, 15, 9, 5), stream_id="s1",
                        project="p", is_error=True),
        ToolResultEvent(ts=_utc(2026, 5, 15, 9, 2), stream_id="s1",
                        project="p", is_error=False),  # success → no timestamp
        ToolResultEvent(ts=_utc(2026, 5, 15, 9, 1), stream_id="s1",
                        project="p", is_error=True),
    ]
    agg = _aggregate_events(date(2026, 5, 15), events)
    assert agg.streams[0].tool_error_count == 2
    assert agg.streams[0].tool_error_timestamps == (
        _utc(2026, 5, 15, 9, 1), _utc(2026, 5, 15, 9, 5),
    )


def test_tool_error_timestamps_survive_cache_round_trip():
    events = [
        ToolResultEvent(ts=_utc(2026, 5, 15, 9, 1), stream_id="s1",
                        project="p", is_error=True),
        ToolResultEvent(ts=_utc(2026, 5, 15, 10, 30), stream_id="s1",
                        project="p", is_error=True),
    ]
    agg = _aggregate_events(date(2026, 5, 15), events)
    restored = _aggregate_from_dict(_aggregate_to_dict(agg))
    assert restored.streams[0].tool_error_timestamps == \
        agg.streams[0].tool_error_timestamps
    assert restored.streams[0].tool_error_timestamps == (
        _utc(2026, 5, 15, 9, 1), _utc(2026, 5, 15, 10, 30),
    )


def test_legacy_aggregate_dict_without_error_timestamps_loads_empty():
    """A cached/archived day written before the field existed restores with an
    empty tuple (and metrics falls back to uniform apportionment for it)."""
    legacy = {
        "day": "2026-05-15",
        "peak_concurrent_streams": 1,
        "streams": [{
            "stream_id": "s1", "project": "p",
            "first_ts": _utc(2026, 5, 15, 9).isoformat(),
            "last_ts": _utc(2026, 5, 15, 10).isoformat(),
            "tool_error_count": 3,
            # no "tool_error_timestamps" key
        }],
    }
    restored = _aggregate_from_dict(legacy)
    assert restored.streams[0].tool_error_count == 3
    assert restored.streams[0].tool_error_timestamps == ()
...
def test_cache_invalidates_on_schema_version_change(tmp_path):
    ...
    """
    from ai_code_cognitive_stress.pipeline.aggregate import CACHE_NAMESPACE
    proj = _project(tmp_path)
    _write_session(proj / "s.jsonl", [
        _session_record("user", "2026-05-15T09:00:00.000Z"),
    ])
...
def test_module_fingerprint_returns_12_lowercase_hex():
    from ai_code_cognitive_stress.pipeline.aggregate import _module_fingerprint
...
def test_cache_path_contains_namespace(tmp_path):
    from ai_code_cognitive_stress.pipeline.aggregate import CACHE_NAMESPACE, _cache_path
...
def test_prune_removes_stale_namespaces_keeps_current(tmp_path):
    ...
    import re
    from ai_code_cognitive_stress.pipeline.aggregate import CACHE_NAMESPACE, _prune_stale_cache_namespaces
...
def test_write_then_read_cache_roundtrip(tmp_path):
    ...
    from ai_code_cognitive_stress.pipeline.aggregate import _read_cache, _write_cache
    cache_dir = tmp_path / "cache"
    day = date(2026, 5, 15)
