...
The actual per-tool parsing lives in `source/adapters/*` —
ClaudeCodeSessionSource, CodexSessionSource. Add new agent-coding tools
by implementing the `SessionSource` protocol
(see `adapters/base.py`); no changes are needed in this module.
...
if TYPE_CHECKING:
    from ..adapters.base import SessionSource
